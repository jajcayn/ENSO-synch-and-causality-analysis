...
def load_enso_SSTs(emr = False, num_ts = 0):
    ...
    if emr:
        ...
        raw = sio.loadmat("Nino34-ERM-1884-2013linear-no-anom-16k-selectedPCs.mat")['N34s']
        enso.data = raw[-16384:, num_ts]
...
for model_no in range(20):
    ...
    if NUM_SURR > 0:
        ...
        if NUM_SURR > 0:
            ...
            fname = "bins/zCMI_%sSST_%dFTsurrs-linear-no-anom-16k-selectedPCs-TSno%d.bin" % (DATA.upper(), NUM_SURR, model_no) 
        ...
        # plt.show()
        plt.savefig('plots/ERM1884-2013-linear-no-anom-16k-selectedPCs_z-score_annual%d.png' % (model_no))
        plt.close()
