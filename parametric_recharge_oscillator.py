...
class ENSO_PROmodel():
    ...
    def integrate_PROmodel(self, sigma = 0.04):
        ...
        if not self.damped:
            ...
            while r.successful and r.t < self.length:
                r.integrate(r.t + dt)
                temp.append(r.y[0])
            ...
            t_tmp = t0
            y = np.zeros((self.length + int(140*self.year), 2))
            y[0, :] = y0
            ...
            if not self.daily:
                for i in range(self.length / 30):
                    ts.append(np.mean(temp[i*30:(i+1)*30]))
            else:
                ts = temp.copy()

        self.data = np.array(ts)
        return np.array(ts)
